...
def print_and_count(channel=CHANNEL):
    ...
    if not playlists:
        print(
            "No playlists returned. Is the channel correct and does it have "
    ...
    if meta.get("incomplete"):
        print(
            "  NOTE: yt-dlp had trouble reading the playlists page, so this\n"
    ...
    if COUNT_VIDEOS:
        total = len(playlists)
        print(
            f"Counting videos in {total} playlist(s) "
    ...
    for p in playlists:
        ...
        print(
            f"  {str(count):>4} videos  [{avail:<8}]  "
            f"{p['id'] or '(no id)':<40}  {p['title']}"
        )

    non_public = [
        p for p in playlists if p["availability"] and p["availability"] != "public"
    ]
    if non_public:
        print(
